...
def indexar():
    if not os.path.exists(dircorr) or not os.path.exists(diragend):
        print "Error: no existe el directorio de documentos: "+dircorr+" "+diragend
    else:
...
def get_schema():
    return Schema(remitente=TEXT(stored=True), destinatarios=KEYWORD(stored=True),fecha=TEXT(stored=True), asunto=TEXT(stored=True), contenido=TEXT(stored=True))
...
def ventana_principal():
    top = Tk()
    indexar = Button(top, text="Indexar", command = indexar)
    indexar.pack(side = TOP)
    Buscar = Button(top, text="Buscar por Remitente")
    Buscar.pack(side = TOP)
    Buscar = Button(top, text="Buscar por Fecha")
    Buscar.pack(side = TOP)
    top.mainloop()
